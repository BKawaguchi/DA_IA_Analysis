...
DA_s_matched_rank_sum = []
IA_s_matched_rank_sum = []
DA_c_matched_rank_sum = []
IA_c_matched_rank_sum = []
...
while count < T:
    ...
    #学生がマッチした研究室を希望順位で表したリストを作成
    DA_s_matched_rank = []
    for i in range(S):
        if DA_s_matched[i]==-1:
            DA_s_matched_rank.append(0)
            
        for j in range(apply_num):
            if DA_s_matched[i]+1==s_prefs[i][j]:
                DA_s_matched_rank.append(j+1)
                
    IA_s_matched_rank = []
    for i in range(S):
        if IA_s_matched[i]==-1:
            IA_s_matched_rank.append(0)
            
        for j in range(apply_num):
            if IA_s_matched[i]+1==s_prefs[i][j]:
                IA_s_matched_rank.append(j+1)

    #第一希望にマッチした学生の数, The number of students getting their first choice
    DA_s_noenvy = 0
    for i in range(S):
        if DA_s_matched[i]+1 == s_prefs[i][0]:
            DA_s_noenvy += 1
            
    IA_s_noenvy = 0
    for i in range(S):
        if IA_s_matched[i]+1 == s_prefs[i][0]:
            IA_s_noenvy += 1
    ...
    IA_k = []
    for i in range(C):
        DA_k.append([n for n, x in enumerate(DA_c_matched[i]) if x==1])
        IA_k.append([n for n, x in enumerate(IA_c_matched[i]) if x==1])
        
    for i in range(C):
        for j in range(len(DA_k[i])):
            for n in range(S):
                if DA_k[i][j]+1==c_prefs[i][n]:
                    DA_c_matched_rank.append(n+1)
                    
    for i in range(C):
        for j in range(len(IA_k[i])):
            for n in range(S):
                if IA_k[i][j]+1==c_prefs[i][n]:
                    IA_c_matched_rank.append(n+1)
    ...
    #1マッチング毎の希望順位ベースのリストを一つにまとめる
    for i in range(S):
        DA_s_matched_rank_sum.append(DA_s_matched_rank[i])
        IA_s_matched_rank_sum.append(IA_s_matched_rank[i])
        
    for i in range(len(DA_c_matched_rank)):
        DA_c_matched_rank_sum.append(DA_c_matched_rank[i])
    
    for i in range(len(IA_c_matched_rank)):
        IA_c_matched_rank_sum.append(IA_c_matched_rank[i])

    count += 1

#ヒストグラムに使用するリストの作成
DA_s_matched_rank_num = []
IA_s_matched_rank_num = []
for i in range(apply_num):
    DA_s_matched_rank_num.append(DA_s_matched_rank_sum.count(i+1))
    IA_s_matched_rank_num.append(IA_s_matched_rank_sum.count(i+1))
    
DA_c_matched_rank_num = []
IA_c_matched_rank_num = []
for i in range(S):
    DA_c_matched_rank_num.append(DA_c_matched_rank_sum.count(i+1))
    IA_c_matched_rank_num.append(IA_c_matched_rank_sum.count(i+1))
    
DA_s_matched_rank_num_ave = []
IA_s_matched_rank_num_ave = []
for i in range(apply_num):
    DA_s_matched_rank_num_ave.append(round(DA_s_matched_rank_num[i]/T))
    IA_s_matched_rank_num_ave.append(round(IA_s_matched_rank_num[i]/T))
    
DA_c_matched_rank_num_ave = []
IA_c_matched_rank_num_ave = []
for i in range(S):
    DA_c_matched_rank_num_ave.append(round(DA_c_matched_rank_num[i]/T))
    IA_c_matched_rank_num_ave.append(round(IA_c_matched_rank_num[i]/T))
    
DA_s_matched_rank_ave = []
for i in range(apply_num):
    for j in range(DA_s_matched_rank_num_ave[i]):
        DA_s_matched_rank_ave.append(i+1)
        
IA_s_matched_rank_ave = []
for i in range(apply_num):
    for j in range(IA_s_matched_rank_num_ave[i]):
        IA_s_matched_rank_ave.append(i+1)
        
DA_c_matched_rank_ave = []
for i in range(S):
    for j in range(DA_c_matched_rank_num_ave[i]):
        DA_c_matched_rank_ave.append(i+1)
        
IA_c_matched_rank_ave = []
for i in range(S):
    for j in range(IA_c_matched_rank_num_ave[i]):
        IA_c_matched_rank_ave.append(i+1)
        
#ヒストグラムの作成
X = DA_s_matched_rank_ave
Y = IA_s_matched_rank_ave
V = DA_c_matched_rank_ave
W = IA_c_matched_rank_ave

fig, ax = plt.subplots()
plt.title('Dispersion of The laboratories')
plt.xlabel('Priorities of The students')
plt.ylabel('Frequency')
plt.xlim(1,max([max(DA_s_matched_rank_ave),max(IA_s_matched_rank_ave)]))
plt.hist([X,Y],label=['DA','IA'])
...
fig, ax = plt.subplots()
plt.title('Dispersion of The students')
plt.xlabel('Priorities of The laboratories')
plt.ylabel('Frequency')
plt.xlim(1,max([max(DA_c_matched_rank_ave),max(IA_c_matched_rank_ave)]))
plt.hist([V,W],label=['DA','IA'])
ax.xaxis.set_major_locator(MaxNLocator(integer=True))
ax.yaxis.set_major_locator(MaxNLocator(integer=True))
plt.legend(loc='upper right')
plt.grid(True)
