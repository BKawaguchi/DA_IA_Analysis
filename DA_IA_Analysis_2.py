...
while count < T:
    ...
    #安定性（ブロッキング・ペアの数）, Stability: The number of Brocking pares
    DA_stability = 0
    for i in range(S):
        for j in range(apply_num):
            if DA_s_matched[i]==s_prefs[i][j]-1:
                break
            elif DA_s_matched[i]!=s_prefs[i][j]-1:
                k=s_prefs[i][j]-1
    ...
    for i in range(S):
        for j in range(apply_num):
            if IA_s_matched[i]==s_prefs[i][j]-1:
                break
            elif IA_s_matched[i]!=s_prefs[i][j]-1:
                k=s_prefs[i][j]-1
                for m in range(capacity[k]):
    ...
    DA_s_anmatch_sum += DA_s_anmatch
    IA_s_anmatch_sum += IA_s_anmatch
...
print('DA: {} 平均値: {} 割合: {}%'.format(DA_s_anmatch_sum, DA_s_anmatch_sum/T, ((DA_s_anmatch_sum/T)/S)*100))
print('IA: {} 平均値: {} 割合: {}%'.format(IA_s_anmatch_sum, IA_s_anmatch_sum/T, ((IA_s_anmatch_sum/T)/S)*100))
print()
...
print('DA: {} 平均値: {} 比較値: {}'.format(DA_s_utility_sum, DA_s_utility_sum/T, (DA_s_utility_sum/T)/S))
print('IA: {} 平均値: {} 比較値: {}'.format(IA_s_utility_sum, IA_s_utility_sum/T, (IA_s_utility_sum/T)/S))
print()
print('研究室の効用, Utility of the laboratories')
print('DA: {} 平均値: {} 比較値: {}'.format(DA_c_utility_sum, DA_c_utility_sum/T, (DA_c_utility_sum/T)/S))
print('IA: {} 平均値: {} 比較値: {}'.format(IA_c_utility_sum, IA_c_utility_sum/T, (IA_c_utility_sum/T)/S))
print()
print('安定性（ブロッキング・ペアの数）, Stability: The number of Brocking pares')
